...
func packProofResult(witnessPayload []byte, proof *groth16bls.Proof, proveMS float64) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decode witness summary: %w", err)
	}
	claimed32 := mustLE32(summary.ClaimedStatementHash)
	buf.Write(claimed32[:])
...
func mustLE32(decimal string) [32]byte {
	value := prototypeMustBigInt(decimal)
	bytes := value.Bytes()
	var out [32]byte
	for i := range bytes {
...
func prototypeMustBigInt(dec string) *big.Int {
...
func penumbra_gnark_spend_init(artifactDir *C.char, artifactDirLen C.size_t, out *C.PenumbraGnarkInitResult) {
	...
	*out = C.PenumbraGnarkInitResult{}
	...
	metadata, err := prototype.LoadCircuitMetadata(dir)
	if err != nil {
