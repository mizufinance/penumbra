...
pub struct SummaryRow {
    ...
    pub broadcast_accepted_tps: f64,
    pub checktx_accepted_tps: f64,
    pub checktx_accepted_total: u64,
    pub checktx_rejected_total: u64,
    pub committed_tps: f64,
    pub reject_rate_pct: f64,
    pub checktx_reject_rate_pct: f64,
    pub p50_ms: f64,
    pub p95_ms: f64,
    pub p99_ms: f64,
    ...
    pub sender_pacing_wait_mean_ms: f64,
    pub sender_inflight_wait_mean_ms: f64,
    pub sender_rpc_wait_mean_ms: f64,
    pub pd_checktx_new_ok_total: u64,
    pub pd_checktx_new_rejected_total: u64,
    pub pd_checktx_new_duration_mean_ms: f64,
    pub pd_checktx_pending_final: f64,
    pub pd_checktx_in_flight_final: f64,
    pub comet_mempool_size_final: f64,
    pub comet_mempool_size_bytes_final: f64,
    pub comet_mempool_recheck_total_final: f64,
    pub git_rev: String,
...
pub fn summarize_case(
    ...
    let sender_pacing_wait_mean_ms =
        mean_submission_metric(submissions, observation, |s| s.pacing_wait_ms);
    let sender_inflight_wait_mean_ms =
        mean_submission_metric(submissions, observation, |s| s.inflight_wait_ms);
    let sender_rpc_wait_mean_ms =
        mean_submission_metric(submissions, observation, |s| s.rpc_wait_ms);
    let submit_mode = scenario.submit_mode.as_str().to_string();
    let checktx_accepted_tps = if scenario.submit_mode == crate::tps::sender::SubmitMode::Sync {
        accepted_tps
    } else {
        0.0
    };
    let checktx_reject_rate_pct = if scenario.submit_mode == crate::tps::sender::SubmitMode::Sync {
        reject_rate_pct
    } else {
        0.0
    };
    ...
    let row = SummaryRow {
        ...
        sent_tps,
        accepted_tps,
        broadcast_accepted_tps: accepted_tps,
        checktx_accepted_tps,
        checktx_accepted_total: 0,
        checktx_rejected_total: 0,
        committed_tps,
        reject_rate_pct,
        checktx_reject_rate_pct,
        p50_ms: latency_p50,
        p95_ms: latency_p95,
        p99_ms: latency_p99,
        ...
        sender_pacing_wait_mean_ms,
        sender_inflight_wait_mean_ms,
        sender_rpc_wait_mean_ms,
        pd_checktx_new_ok_total: 0,
        pd_checktx_new_rejected_total: 0,
        pd_checktx_new_duration_mean_ms: 0.0,
        pd_checktx_pending_final: 0.0,
        pd_checktx_in_flight_final: 0.0,
        comet_mempool_size_final: 0.0,
        comet_mempool_size_bytes_final: 0.0,
        comet_mempool_recheck_total_final: 0.0,
        git_rev: git_rev(),
...
fn mean_submission_metric(
    submissions: &[SubmissionRecord],
    observation: &ObservationOutput,
    metric: impl Fn(&SubmissionRecord) -> f64,
) -> f64 {
