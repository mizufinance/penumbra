...
| optimization preserves byte trace or versions the protocol | `crates/crypto/proof-aggregation/src/backend.rs`; `crates/crypto/proof-aggregation-reference/src/lib.rs`; `crates/crypto/proof-aggregation/src/ipp/dh_commitments/src/afgho16/mod.rs`; `crates/crypto/proof-aggregation/src/ipp/ip_proofs/src/gipa.rs` | committed aggregate-byte and PenumbraByte-trace baselines; `msm_keys` final commitment-key recombination | Rust golden-baseline tests plus unit equivalence | `aggregate_bytes_match_committed_baseline`; `penumbra_byte_trace_matches_committed_baseline`; `msm_keys_equals_sequential_fold`; committed fixtures under `tests/fixtures/` | aggregate bytes and transcript are locked to a version-tagged baseline; an optimization either preserves them or bumps `AGGREGATE_PROTOCOL_VERSION` with an adaptation-register row | composed | n/a | test gates passed |
