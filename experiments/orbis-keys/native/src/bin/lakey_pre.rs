...
use orbis_crypto::r#trait::{DistKeyShare, PriShare, PubPoly, ThresholdDealer};
use orbis_crypto::{CryptoSerialize, PreImpl, PubPolyImpl};
use rand::rngs::OsRng;
use serde_json::json;
...
fn main() {
    ...
    let logq: usize = args.get(5).map(|x| x.parse().unwrap()).unwrap_or(12);
    let stat: usize = args.get(6).map(|x| x.parse().unwrap()).unwrap_or(40);
    ...
    .enumerate()
    {
        ...
        h.update(b"shieldd-lakey-experiment-v1\0");
    ...
    for _ in 0..32 {
        ...
        let bytes = epk.to_bytes().unwrap();
        let mut opened = Vec::new();
        for slot in 0..4 {
            let mut replies = Vec::new();
            for (i, s) in by_node.iter().take(t).enumerate() {
                let sk = DistKeyShare {
                    pri_share: PriShare {
                        i: i as u32 + 1,
                        v: s[slot],
                ...
                let reply = pre
                    .reencrypt_commitment(&sk, &bytes, &reader_pk, &pop, None)
                    .unwrap();
                pre.verify(&reader_pk, &polys[slot], &epk, &reply, None)
                    .unwrap();
                assert!(pre
                    .verify(&reader_pk, &polys[(slot + 1) % 4], &epk, &reply, None)
                    .is_err());
                assert!(pre
                    .verify(
                        &reader_pk,
            ...
            let combined = pre.recover(&replies, t, n).unwrap().unwrap();
            let ss = combined - polys[slot].commits[0] * reader;
            assert_eq!(ss, polys[slot].commits[0] * r);
            opened.push(ss);
        }
        // Reuse the same ephemeral point deliberately: identity/field/mode isolation
        // must come from unrelated derived keys, not merely fresh ciphertexts.
        for i in 1..4 {
            assert_ne!(opened[0], opened[i]);
        }
        let da = PreImpl::derive_capability_scalar(b"Alice");
        let db = PreImpl::derive_capability_scalar(b"Bob");
        assert_ne!(opened[0] * db * da.inverse().unwrap(), opened[1]);
    }
    println!(
        "{}",
        json!({"case":"LaKey MPC shares into pinned Orbis Decaf377 PRE","nodes":n,"threshold":t,"ciphertexts":32,"independent_scopes":4,"seconds":start.elapsed().as_secs_f64(),"persistent_master_scalar_shares_per_node":512,"clear_key_output":false,"fixture_reference_reconstructs_synthetic_master_in_test_process":true,"checks":"independent clear LaKey reference, polynomial consistency, restart/refresh public keys, DLEQ, wrong key/field/ephemeral point, insufficient shares, public-ratio attack"})
    );
}
