        ...
        match self {
            ...
            Self::Export {
                ...
                if *payload_keys {
                    ...
                    eprintln!("Experimental proof privacy: RISC Zero's security model cautions users with critical privacy requirements.");
                }
...
async fn run_worker(
    ...
    command
        ...
        .env_remove("RISC0_DUMP_PATH")
        .env_remove("RISC0_PPROF_OUT")
        .env_remove("RISC0_PPROF_ENABLE_INLINE_FUNCTIONS")
        .kill_on_drop(true);
...
mod tests {
    ...
    async fn prover_stdout_does_not_corrupt_worker_result() {
        ...
        let package = sdk::DisclosurePackage {
            ...
            evidence: sdk::Evidence::ZkReceipt(vec![0]),
            context: vec![],
        };
        std::fs::write(
            &executable,
    ...
    async fn worker_suppresses_dumps_and_cancels_process_group() {
        ...
        std::fs::write(
            &executable,
            b"#!/bin/sh\n[ -z \"${RISC0_DUMP_PATH+x}${RISC0_PPROF_OUT+x}${RUST_LOG+x}${RISC0_DEV_MODE+x}\" ] || exit 1\nprintf '%s' $$ > \"$0.pid\"\nsleep 30 &\nwait\n",
        )
        .unwrap();
        std::fs::set_permissions(&executable, std::fs::Permissions::from_mode(0o700)).unwrap();
