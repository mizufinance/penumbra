...
mod tests {
    ...
    // Throwaway benchmark (run with: cargo test --release -p penumbra-sdk-proof-aggregation
    //   subgroup_vs_deserialize_bench -- --ignored --nocapture). Not part of the gate set.
    #[test]
    #[ignore]
    fn subgroup_vs_deserialize_bench() {
        ...
        // ---- Part 1: per-element GT subgroup check vs one batched randomized check ----
        let mut rng = ChaCha20Rng::seed_from_u64(1);
        eprintln!("\n=== Part 1: GT subgroup validation (per-element vs batched) ===");
        eprintln!("{:>6} | {:>14} | {:>14} | {:>7}", "N", "per-elem (ms)", "batched (ms)", "ratio");
        for &n_gt in &[16usize, 28, 64] {
            // In-subgroup GT elements = outputs of real pairings.
            let elems: Vec<PairingOutput<Bls12_377>> = (0..n_gt)
                .map(|_| {
                    let g1 = <Bls12_377 as Pairing>::G1::rand(&mut rng).into_affine();
                    let g2 = <Bls12_377 as Pairing>::G2::rand(&mut rng).into_affine();
                    Bls12_377::pairing(g1, g2)
                })
                .collect();
            // 128-bit verifier-local randomizers (the realistic security choice).
            let scalars: Vec<[u64; 2]> =
                (0..n_gt).map(|_| [rng.next_u64(), rng.next_u64()]).collect();
            ...
            let t = Instant::now();
            for _ in 0..iters {
                let mut acc = PairingOutput::<Bls12_377>::default();
                for (e, s) in elems.iter().zip(&scalars) {
                    acc += e.mul_bigint(s);
                }
                acc.check().expect("batch in subgroup");
            }
            let batch_ms = t.elapsed().as_secs_f64() * 1000.0 / iters as f64;
            ...
            eprintln!(
                ...
                per_elem_ms,
                batch_ms,
                per_elem_ms / batch_ms
            );
        }

        // ---- Part 2: deserialize_ms as a share of total verify work ----
        eprintln!("\n=== Part 2: deserialize share of SnarkPack verify (RAYON_NUM_THREADS controls cores) ===");
        eprintln!(
            "{:>6} | {:>13} | {:>13} | {:>9} | {:>9}",
            "count", "deserial(ms)", "core_tot(ms)", "deser %", "ppe(ms)"
        );
        let srs = DevSrs::default();
        let (pvk, items) = sample_items_with_count(7, 64);
        let base = items.first().unwrap().clone();
        for &count in &[1usize, 2, 4, 8, 64] {
            let repeated = vec![base.clone(); count];
            let statement =
                statement_for_items(ProofFamilyId::Transfer, &pvk, count, &repeated, &srs);
            let aggregate =
                aggregate_family(&statement, &pvk, &repeated, &srs).expect("aggregate");
            // Average a few profiled verifies.
            let runs = 20u32;
            let (mut des, mut core, mut ppe) = (0.0f64, 0.0f64, 0.0f64);
            for _ in 0..runs {
                let p = verify_family_aggregate_profiled(&statement, &pvk, &aggregate, &srs)
                    .expect("verify");
                des += p.deserialize_ms;
                core += p.core_total_ms;
                ppe += p.ppe_ms;
            }
            let (des, core, ppe) = (des / runs as f64, core / runs as f64, ppe / runs as f64);
            eprintln!(
                "{:>6} | {:>13.4} | {:>13.4} | {:>8.1}% | {:>9.4}",
                count,
                des,
