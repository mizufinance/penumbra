...
use crate::{
    ...
    shielded_withdrawal::{
        ShieldedWithdrawalChangePrivate, ShieldedWithdrawalChangePublic,
        ShieldedWithdrawalInputPublic, ShieldedWithdrawalProofPrivate,
...
pub struct ShieldedWithdrawalRequiredSpendWitness {
    pub nullifier: [u8; 32],
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ShieldedWithdrawalOptionalSpendWitness {
    pub spend: ShieldedWithdrawalRequiredSpendWitness,
    pub is_dummy: bool,
    pub dummy_nullifier_seed: [u8; 32],
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ShieldedWithdrawalChangeWitness {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ShieldedWithdrawalWitness {
    ...
    pub required_spend: ShieldedWithdrawalRequiredSpendWitness,
    pub optional_spend: ShieldedWithdrawalOptionalSpendWitness,
    pub change_output: ShieldedWithdrawalChangeWitness,
...
fn spend_witness(
    public_input: &ShieldedWithdrawalInputPublic,
    private_input: &ShieldedWithdrawalRequiredInputPrivate,
    index: usize,
) -> Result<ShieldedWithdrawalRequiredSpendWitness> {
    ...
    Ok(ShieldedWithdrawalRequiredSpendWitness {
        ...
        rk_affine: point_affine_bytes(verification_key_point(
            public_input.rk,
            &format!("shielded_withdrawal_rk_{index}"),
        )?)?,
...
fn change_witness(
    public_output: &ShieldedWithdrawalChangePublic,
    private_output: &ShieldedWithdrawalChangePrivate,
    capk: decaf377::Element,
) -> Result<ShieldedWithdrawalChangeWitness> {
    Ok(ShieldedWithdrawalChangeWitness {
...
impl ShieldedWithdrawalWitness {
    pub fn from_public_private(
        ...
        if public.family_id != private.family_id {
            bail!(
                "shielded withdrawal witness family mismatch: public={} private={}",
        ...
        let input_total = required_amount
            ...
            .ok_or_else(|| anyhow!("shielded withdrawal input amount sum overflows u128"))?;
        let output_total = change_amount
            .checked_add(outbound_amount)
            .ok_or_else(|| anyhow!("shielded withdrawal output amount sum overflows u128"))?;
        if input_total != output_total {
            bail!(
                "shielded withdrawal must conserve value internally: inputs={input_total}, outputs={output_total}"
            );
        }
        ...
        if public.balance_commitment != expected_balance_commitment {
            bail!("shielded withdrawal balance commitment must be the blinding-only commitment");
        }

        let claimed_statement_hash = shielded_withdrawal_statement_hash_from_public(public)
        ...
        let optional_spend = ShieldedWithdrawalOptionalSpendWitness {
