...
use crate::{
    gnark::{
        ...
        shielded_withdrawal_witness::{
            ShieldedWithdrawalChangeWitness, ShieldedWithdrawalOptionalSpendWitness,
            ShieldedWithdrawalRequiredSpendWitness, ShieldedWithdrawalWitness,
        },
...
impl ShieldedWithdrawalWitness {
    pub fn encode(&self) -> Result<Vec<u8>> {
        let mut buf = Vec::new();
        put_bytes(&mut buf, SHIELDED_WITHDRAWAL_WITNESS_MAGIC);
        put_u32(&mut buf, 0);
        ...
        encode_point_affine(&mut buf, &self.withdrawal_epk_affine);
        put_bytes(&mut buf, &self.withdrawal_c2);
        put_bytes(&mut buf, &self.withdrawal_key_confirmation);
        for word in &self.withdrawal_encrypted_sender_address {
        ...
        let total_len =
            u32::try_from(buf.len()).context("encoded shielded withdrawal witness exceeds u32")?;
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        if cursor.read_fixed::<4>()? != *SHIELDED_WITHDRAWAL_WITNESS_MAGIC {
            bail!("invalid shielded withdrawal witness magic");
        }
        let total_length = cursor.read_u32()?;
        if total_length as usize != bytes.len() {
            bail!(
                "shielded withdrawal witness length mismatch: header={}, actual={}",
                total_length,
                bytes.len()
            );
        }

        let family_id = ShieldedWithdrawalFamilyId::try_from(cursor.read_u32()?)?;
        ...
        let witness = Self {
            ...
            withdrawal_epk_affine: cursor.read_point_affine()?,
            withdrawal_c2: cursor.read_fixed::<32>()?,
            withdrawal_key_confirmation: cursor.read_fixed::<32>()?,
            withdrawal_encrypted_sender_address: [
...
fn encode_required_spend(
    buf: &mut Vec<u8>,
    spend: &ShieldedWithdrawalRequiredSpendWitness,
) -> Result<()> {
...
fn encode_optional_spend(
    buf: &mut Vec<u8>,
    spend: &ShieldedWithdrawalOptionalSpendWitness,
) -> Result<()> {
...
fn decode_optional_spend(
    cursor: &mut BinaryCursor<'_>,
) -> Result<ShieldedWithdrawalOptionalSpendWitness> {
    Ok(ShieldedWithdrawalOptionalSpendWitness {
...
fn encode_change_output(buf: &mut Vec<u8>, output: &ShieldedWithdrawalChangeWitness) {
...
fn decode_change_output(cursor: &mut BinaryCursor<'_>) -> Result<ShieldedWithdrawalChangeWitness> {
