...
impl MsgHandler for MsgUpgradeClient {
    async fn check_stateless<AH>(&self) -> Result<()> {
        let cs = AnyClientState::try_from(self.client_state.clone())
            .context("MsgUpgradeClient: unsupported client state type")?;
        AnyConsensusState::try_from(self.consensus_state.clone())
            .context("MsgUpgradeClient: unsupported consensus state type")?;

        if matches!(cs, AnyClientState::Bankd(_)) {
            anyhow::bail!("bankd client upgrades are not supported");
        }
    ...
    async fn try_execute<S: StateWrite, AH, HI: HostInterface>(&self, mut state: S) -> Result<()> {
        ...
        let upgraded_client_state = AnyClientState::try_from(self.client_state.clone())
            .context("failed to decode upgraded client state")?;
        let upgraded_consensus_state = AnyConsensusState::try_from(self.consensus_state.clone())
            .context("failed to decode upgraded consensus state")?;

        match (upgraded_client_state, upgraded_consensus_state) {
            (
                AnyClientState::Tendermint(upgraded_client_state_tm),
                AnyConsensusState::Tendermint(upgraded_consensus_state_tm),
            ) => {
                ...
                state.put_client(
                    &self.client_id,
                    AnyClientState::Tendermint(new_client_state),
                );
                state
            ...
            (AnyClientState::Bankd(_), _) => {
                anyhow::bail!("bankd client upgrades are not supported");
            }
            _ => {
                anyhow::bail!("mismatched client state and consensus state types in upgrade");
            }
        }
