...
#[ignore = "builds an accepted payment and generates a real local RISC Zero disclosure"]
async fn export_import_between_wallet_directories() -> Result<()> {
    run_disclosure(true).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "builds an accepted payment and executes its disclosure guest without proving"]
async fn accepted_disclosure_guest_execution() -> Result<()> {
...
async fn run_disclosure(prove: bool) -> Result<()> {
    ...
    let metadata = disclosure::MetadataDocument {
        entries: vec![
            disclosure::MetadataEntry {
            ...
            disclosure::MetadataEntry {
                ...
                output: 0,
                name: "private".into(),
                value: "UNSHARED-METADATA".into(),
            },
        ],
    };
    let memo = wallet
        .prepare_disclosure_metadata(metadata, test_keys::ADDRESS_0.clone())
        .await?;
    ...
    let request = disclosure::DisclosureRequest {
        ...
        challenge: Some("request-42".into()),
        total: None,
    ...
    let session = ExternalProver::new("local", "r0vm").execute(
        ExecutorEnv::builder()
            .write_slice(&input)
    ...
    let guest: disclosure::DisclosureStatement = serde_json::from_slice(&session.journal.bytes)?;
    ensure!(
        guest == statement,
        "accepted-payment guest and host disagree"
    );
    eprintln!(
        "Accepted-payment guest: {} cycles, {} segments",
        session.cycles(),
    ...
    let disclosure::Evidence::ZkReceipt(receipt_bytes) = &package.evidence else {
    ...
    let verify_started = std::time::Instant::now();
        ...
        disclosure::verify(&package)?.cryptography_verified,
        "invalid receipt"
    );
    eprintln!("Local receipt verification: {:?}", verify_started.elapsed());
    let public = serde_json::to_string(&package.statement)?;
        ...
        !String::from_utf8_lossy(&bytes).contains("UNSHARED-METADATA"),
        "private metadata leaked"
    );
    ensure!(
