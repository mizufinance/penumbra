...
fn main() -> anyhow::Result<()> {
    ...
    let input_dir = PathBuf::from(args.next().expect("input directory"));
    let target_dir = PathBuf::from(args.next().expect("output directory"));
    let roots: Vec<PathBuf> = args.map(|path| input_dir.join(path)).collect();
    anyhow::ensure!(!roots.is_empty(), "protobuf roots are required");
