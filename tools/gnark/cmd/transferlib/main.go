...
const (
	transferProofResultMagic   = "PTPR"
	transferProofResultVersion = 1
)
...
func packProofResult(witnessPayload []byte, proof *groth16bls.Proof, proveMS float64) ([]byte, error) {
	...
	buf.WriteString(transferProofResultMagic)
	_ = binary.Write(buf, binary.LittleEndian, uint32(transferProofResultVersion))
