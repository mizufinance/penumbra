...
const (
	splitProofResultMagic   = "PLPR"
	splitProofResultVersion = 1
)
...
func packProofResult(witnessPayload []byte, proof *groth16bls.Proof, proveMS float64) ([]byte, error) {
	...
	buf.WriteString(splitProofResultMagic)
	_ = binary.Write(buf, binary.LittleEndian, uint32(splitProofResultVersion))
	...
	*out = C.PenumbraGnarkBytesResult{}
	...
	if family.ID != ctx.familyID {
		setBytesResult(
			out,
			1,
			[]byte(fmt.Sprintf("split witness family mismatch: got %s (%d), expected %s (%d)", family.Label, family.ID, ctx.circuitName, ctx.familyID)),
			0,
		)
		return
	}
