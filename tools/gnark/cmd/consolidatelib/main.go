...
//export penumbra_gnark_consolidate_init
func penumbra_gnark_consolidate_init(artifactDir *C.char, artifactDirLen C.size_t, out *C.PenumbraGnarkInitResult) {
	if out == nil {
		return
	}
	logger.Disable()
...
func penumbra_gnark_consolidate_init_from_bytes(
...
func penumbra_gnark_consolidate_prove(handle C.uint64_t, witnessPtr unsafe.Pointer, witnessLen C.size_t, out *C.PenumbraGnarkBytesResult) {
	if out == nil {
		return
	}
	logger.Disable()
