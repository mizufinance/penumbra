...
const (
	outputProofResultMagic   = "POPR"
	outputProofResultVersion = 1
)
...
func packProofResult(witnessPayload []byte, proof *groth16bls.Proof, proveMS float64) ([]byte, error) {
	...
	buf.WriteString(outputProofResultMagic)
	_ = binary.Write(buf, binary.LittleEndian, uint32(outputProofResultVersion))
	_ = binary.Write(buf, binary.LittleEndian, uint32(0))
	_ = binary.Write(buf, binary.LittleEndian, uint32(0))
	_ = binary.Write(buf, binary.LittleEndian, uint64(proveMS*1000))

	witness, err := abi.DecodeOutputWitnessV1(witnessPayload)
	if err != nil {
		return nil, fmt.Errorf("decode output witness: %w", err)
	}
...
//export penumbra_gnark_output_init
func penumbra_gnark_output_init(artifactDir *C.char, artifactDirLen C.size_t, out *C.PenumbraGnarkInitResult) {
	if out == nil {
		return
	}
	*out = C.PenumbraGnarkInitResult{}
	...
	if err != nil {
		setError(out, fmt.Errorf("compile output circuit: %w", err))
		return
	}
	...
	if err := artifacts.ValidateCircuitMetadataForCircuit(metadata, "output", ccs); err != nil {
		setError(out, err)
		return
	}
...
//export penumbra_gnark_output_init_from_bytes
func penumbra_gnark_output_init_from_bytes(
	...
	*out = C.PenumbraGnarkInitResult{}
	...
	metadata, err := artifacts.LoadCircuitMetadataBytes(
		...
		"bundled output circuit_metadata.json",
	)
	if err != nil {
...
//export penumbra_gnark_output_prove
func penumbra_gnark_output_prove(handle C.uint64_t, witnessPtr unsafe.Pointer, witnessLen C.size_t, out *C.PenumbraGnarkBytesResult) {
	if out == nil {
		return
	}
	*out = C.PenumbraGnarkBytesResult{}
	...
	if err != nil {
		setBytesResult(out, 1, []byte(fmt.Sprintf("prove output: %v", err)), proveMS)
		return
	}
...
//export penumbra_gnark_output_free
func penumbra_gnark_output_free(ptr unsafe.Pointer, _ C.size_t) {
	if ptr != nil {
		C.free(ptr)
	}
}

//export penumbra_gnark_output_shutdown
func penumbra_gnark_output_shutdown(handle C.uint64_t) {
	contextMu.Lock()
