...
ARTIFACT_DIR="$REPO_DIR/tmp/gnark/spend"
VERIFY_OUT_DIR="$REPO_DIR/tmp/gnark/verify-bench"
WITNESS_BIN="$ROOT_DIR/vectors/spend_witness_v1.bin"
FIXTURE_JSON="$ROOT_DIR/vectors/spend_fixture.json"
PROOF_JSON="$VERIFY_OUT_DIR/spend_artifacts.json"
...
if [[ ! -f "$ARTIFACT_DIR/proving_key.bin" || ! -f "$ARTIFACT_DIR/verifying_key.bin" || ! -f "$ARTIFACT_DIR/circuit_metadata.json" ]]; then
  ...
  (
    cd "$ROOT_DIR"
    go run ./cmd/gnarkctl setup --circuit spend --out-dir "$ARTIFACT_DIR"
  )
fi
...
(
  cd "$ROOT_DIR"
  go run ./cmd/gnarkctl verify-bench \
