...
func TestSpendWitnessV1DecodesAndMatchesFixture(t *testing.T) {
	...
	if int(witness.TotalLength) != len(primitives.LoadSpendWitnessV1()) {
		t.Fatalf("witness length mismatch: got %d want %d", witness.TotalLength, len(primitives.LoadSpendWitnessV1()))
	}
...
func TestSpendWitnessV1RejectsBadHeader(t *testing.T) {
	payload := append([]byte(nil), primitives.LoadSpendWitnessV1()...)
	payload[0] = 'X'
...
func TestReadVec32RejectsOversizedLength(t *testing.T) {
	var payload [4]byte
	binary.LittleEndian.PutUint32(payload[:], maxVec32Length+1)
	if _, err := readVec32(bytes.NewReader(payload[:])); err == nil {
		t.Fatalf("expected oversized vec32 length to fail")
	}
}

func TestReadTriplePathRejectsOversizedLength(t *testing.T) {
	var payload [4]byte
	binary.LittleEndian.PutUint32(payload[:], maxTriplePathLength+1)
	if _, err := readTriplePath(bytes.NewReader(payload[:])); err == nil {
		t.Fatalf("expected oversized triple path length to fail")
	}
}

func TestReadMerklePathRejectsOversizedLayerCount(t *testing.T) {
	var payload [4]byte
	binary.LittleEndian.PutUint32(payload[:], maxMerklePathLayers+1)
	if _, err := readMerklePath(bytes.NewReader(payload[:])); err == nil {
		t.Fatalf("expected oversized merkle path layer count to fail")
	}
}

func TestReadMerklePathRejectsOversizedSiblingCount(t *testing.T) {
	payload := make([]byte, 8)
	binary.LittleEndian.PutUint32(payload[0:4], 1)
	binary.LittleEndian.PutUint32(payload[4:8], maxMerklePathSiblings+1)
	if _, err := readMerklePath(bytes.NewReader(payload)); err == nil {
		t.Fatalf("expected oversized merkle path sibling count to fail")
	}
}
