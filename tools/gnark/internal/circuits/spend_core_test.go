...
type compressedHelperEquivalenceCircuit struct {
	NoteBlinding       frontend.Variable
	NoteAmount         frontend.Variable
	NoteAssetID        frontend.Variable
	DiversifiedGenX    frontend.Variable
	DiversifiedGenY    frontend.Variable
	TransmissionKeyS   frontend.Variable
	TransmissionPointX frontend.Variable
	TransmissionPointY frontend.Variable
	ClueKey            frontend.Variable
	UserD              frontend.Variable
}

func (c *compressedHelperEquivalenceCircuit) Define(api frontend.API) error {
	divGen := gnarkte.Point{X: c.DiversifiedGenX, Y: c.DiversifiedGenY}
	transmissionPoint := gnarkte.Point{X: c.TransmissionPointX, Y: c.TransmissionPointY}
	divGenFq, err := primitives.Decaf377CompressToField(api, divGen)
	if err != nil {
		return err
	}
	transmissionPointFq, err := primitives.Decaf377CompressToField(api, transmissionPoint)
	if err != nil {
		return err
	}

	noteA, err := NoteCommitment(api, c.NoteBlinding, c.NoteAmount, c.NoteAssetID, divGen, c.TransmissionKeyS, c.ClueKey)
	if err != nil {
		return err
	}
	noteB, err := NoteCommitmentWithCompressedDivGen(api, c.NoteBlinding, c.NoteAmount, c.NoteAssetID, divGenFq, c.TransmissionKeyS, c.ClueKey)
	if err != nil {
		return err
	}
	api.AssertIsEqual(noteA, noteB)

	leafA, err := ComplianceLeafCommitment(api, divGen, transmissionPoint, c.NoteAssetID, c.UserD)
	if err != nil {
		return err
	}
	leafB, err := ComplianceLeafCommitmentFromCompressed(api, divGenFq, transmissionPointFq, c.NoteAssetID, c.UserD)
	if err != nil {
		return err
	}
	api.AssertIsEqual(leafA, leafB)
	return nil
}

func TestCompressedHelperVariantsMatchOriginalHelpers(t *testing.T) {
	fixture, err := primitives.LoadSpendFixture()
	if err != nil {
		t.Fatalf("load spend fixture: %v", err)
	}
	...
	assignment := &compressedHelperEquivalenceCircuit{
		NoteBlinding:       fixture.Private.NoteBlinding,
		NoteAmount:         fixture.Private.NoteAmount,
		NoteAssetID:        fixture.Private.NoteAssetID,
		DiversifiedGenX:    fixture.Private.DiversifiedGeneratorAffine.X,
		DiversifiedGenY:    fixture.Private.DiversifiedGeneratorAffine.Y,
		TransmissionKeyS:   transmissionKeyS.String(),
		TransmissionPointX: fixture.Private.UserTransmissionKeyAffine.X,
		TransmissionPointY: fixture.Private.UserTransmissionKeyAffine.Y,
		ClueKey:            fixture.Private.ClueKey,
	...
	assert := test.NewAssert(t)
	assert.CheckCircuit(
		&compressedHelperEquivalenceCircuit{},
		test.WithCurves(ecc.BLS12_377),
		test.WithBackends(backend.GROTH16),
		test.WithValidAssignment(assignment),
	)
...
func (c *balanceCommitmentCircuit) Define(api frontend.API) error {
	...
	primitives.AssertDecafEquivalent(api, commitment, gnarkte.Point{X: c.ExpectedX, Y: c.ExpectedY})
	return nil
}
...
func (c *randomizedVerificationKeyCircuit) Define(api frontend.API) error {
	...
	primitives.AssertDecafEquivalent(api, rk, gnarkte.Point{X: c.ExpectedX, Y: c.ExpectedY})
	return nil
}
...
func (c *diversifiedTransmissionKeyCircuit) Define(api frontend.API) error {
	...
	primitives.AssertDecafEquivalent(api, pk, gnarkte.Point{X: c.ExpectedX, Y: c.ExpectedY})
	return nil
}
